...
public abstract class Camera {
    ...
    public static int getDiferenceX(){
        return (int) (-fish.getX() + Camera.x+Game.getWIDTH()/2);
    }
    ...
    public static void follow(Fish fish) {
        ...
        float difX = Camera.x+Game.getWIDTH()/2 -fish.getX();
        float difY = Camera.y+Game.getHEIGHT()/2 -fish.getY();
...
//        }
        ...
        if(x<0){x=0;}
        if(y<0){y=0;}
        if(y>(GraphicsPanel.getBackground()[0].getHeight()-Game.getHEIGHT())){y=(GraphicsPanel.getBackground()[0].getHeight()-Game.getHEIGHT());}
        if(x>12000 - Game.getWIDTH()){x= 12000 - Game.getWIDTH();}
    }

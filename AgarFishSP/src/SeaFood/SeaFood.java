...
public abstract class SeaFood {
    ...
    private static ArrayList<SeaFood> seaFoodList = new ArrayList<>();
...
//            y+=BONES_SPEED;
    ...
    public void draw(Graphics g){
        ...
        if((!(x>(Camera.getX()-image[0].getWidth()) && x<(Camera.getX()+Game.getWIDTH()))) || 
            (!(y>(Camera.getY()-image[0].getHeight()) && y<(Camera.getY()+Game.getHEIGHT())))){
            return;
        }
